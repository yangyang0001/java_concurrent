...
class Random implements Serializable {
     ...
     *
     * 子类应该覆盖它，因为它被所有其他方法使用。
     * Subclasses should override this, as this is used by all other methods.
     *
     * <p>
     * {@code next} 的一般约定是它返回一个 {@code int} 值，如果参数 {@code bits} 在 {@code 1} 和 {@code 32} 之间（含），那么很多低 返回值的顺序位将是（大约）独立选择的位值，每个位值（大约）是 {@code 0} 或 {@code 1} 的可能性相同。
     ...
     *
     * {@code next} 方法由 {@code Random} 类实现，通过原子地将种子更新为
     * The method {@code next} is implemented by class {@code Random} by atomically updating the seed to
     * <pre>                123456789
     ...
     * </pre>
     *
     * 这是一个线性同余伪随机数生成器，由 D. H. Lehmer 定义并由 Donald E. Knuth 在计算机编程的艺术，第 3 卷：半数值算法
     ...
     * @return the next pseudorandom value from this random number generator's sequence     // 来自这个随机数生成器序列的下一个伪随机值
     * @since  1.1
     */
    ...
    /**
     * 返回一个伪随机、均匀分布的 {@code int} 值，介于 0（包含）和指定值（不包含）之间，从该随机数生成器的序列中抽取。
     ...
     *
     * {@code nextInt} 的一般约定是 伪随机生成 并 返回指定范围内的 一个 {@code int} 值。
     * The general contract of {@code nextInt} is that one {@code int} value in the specified range is pseudorandomly generated and returned.
     *
     * 所有 {@code bound} 可能的 {@code int} 值都以（近似）相等的概率产生。
     * All {@code bound} possible {@code int} values are produced with (approximately) equal probability.
     *
     * 方法 {@code nextInt(int bound)} 由类 {@code Random} 实现，就像通过：
     * The method {@code nextInt(int bound)} is implemented by class {@code Random} as if by:
     * <pre>
     * {@code
     ...
     *
     * <p>
     * 在前面的描述中使用对冲“大约”只是因为下一个方法只是大约独立选择位的无偏源。
     ...
     *
     * 如果它是随机选择位的完美来源，那么所示算法将从具有完美一致性的所述范围中选择 {@code int} 值。
     * If it were a perfect source of randomly chosen bits, then the algorithm shown would choose {@code int} values from the stated range with perfect uniformity.
     * <p>
     * 该算法有点棘手。
     * The algorithm is slightly tricky.
     *
     * 它拒绝会导致不均匀分布的值（因为 2^31 不能被 n 整除）。
     ...
     *
     * 最坏的情况是 n=2^30+1，拒绝的概率为 1/2，循环终止前的预期迭代次数为 2。
     ...
     *
     * 在没有特殊处理的情况下，将返回正确数量的<i>低位</i> 位。
     * In the absence of special treatment, the correct number of <i>low-order</i> bits would be returned.
     *
     * 众所周知，线性同余伪随机数生成器（例如由此类实现的生成器）在其低位值序列中具有短周期。
     ...
     *
     * 因此，如果 n 是 2 的小幂，这种特殊情况会大大增加连续调用此方法返回的值序列的长度。
